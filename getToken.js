...
(async () => {
    const emailMainAccount = '';
    const loginResult = await registerApps(emailMainAccount);

    if (loginResult.code === 0) {
        console.log('Logining main account!');
        const verifierToken = loginResult.data.verifier;

        console.log('Check email dan konfirmasikan sendiri anjays!, nanti tokennya muncul sendiri')
        ...
        console.log(`Token mu maseh ${JSON.stringify(checkVerifiedResult.data)}`);
        // console.log('Email confirmed!')
        // fs.writeFileSync('./mainAccount.json', JSON.stringify(checkVerifiedResult.data, 0, 2))
        // accessTokenMainAccount = checkVerifiedResult.data.access_token;
        // const detailUser = await getDetailUser(accessTokenMainAccount);
        // inviteCode = detailUser.data.my_invite_code;
    }
})();